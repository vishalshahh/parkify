...
async function main() {
  ...
  const company = await prisma.company.upsert({
    ...
    update: {
      ...
      description:
        'Main parking company managing all garages across multiple global locations',
    },
    create: {
      id: 1,
      displayName: 'Parkify Inc.',
  ...
  console.log(
    `🏢 Seeding ${allLocationGarages.length} garages across 15 global locations...`,
  )
  ...
  for (const garage of allLocationGarages) {
    try {
      ...
      console.error(
        `❌ Error creating garage "${garage.displayName}":`,
        error.message,
      )
      skippedCount++
    }
